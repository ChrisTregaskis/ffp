...
10. **Run the gates** before presenting: `pnpm typecheck`, `pnpm lint` (0 warnings), **`pnpm format:check`**, `pnpm test` (as relevant), `pnpm build` if package boundaries changed. **`lint` does not cover formatting here** — `format:check` is part of the set, not an extra.
