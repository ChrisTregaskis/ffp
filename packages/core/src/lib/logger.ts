 ...
 *
 * @deprecated Use `createLogger(context, minLogLevel?)` instead for a functional approach.
 * This class wrapper is provided for backward compatibility with existing code.
 *
 * @example
 * ```typescript
 ...
 * ```
 */
export class Logger implements TenantLogger {
  private readonly loggerInstance: TenantLogger;

  constructor(context: TenantContext, minLogLevel?: LogLevel) {
    this.loggerInstance = createLogger(context, minLogLevel);
  }
...
/**
 * SystemLogger class for backward compatibility
 *
 * @deprecated Use `createSystemLogger(service, minLogLevel?, requestId?)` instead for a functional approach.
 * This class wrapper is provided for backward compatibility with existing code.
 *
 * @example
 * ```typescript
 * // Old usage (deprecated):
 * const logger = new SystemLogger('job-processor');
 *
 * // New usage (preferred):
 * const logger = createSystemLogger('job-processor');
 * ```
 */
export class SystemLogger implements SystemLoggerInstance {
  private readonly loggerInstance: SystemLoggerInstance;

  constructor(service: string, minLogLevel?: LogLevel, requestId?: string) {
    this.loggerInstance = createSystemLogger(service, minLogLevel, requestId);
  }
