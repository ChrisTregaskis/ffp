...
- **A step's `templateId` cannot be cleared through the update endpoint.** `updateFlowStepSchema` is a plain `.partial()`, so an omitted `templateId` is left alone and there is no nullable form to clear it. Switching a step from `questions` to a type that takes no template therefore leaves the old `template_id` on the row. Harmless today — the authoring surface only reads it for the types that link one — but it means the column is not a reliable answer to "does this step use a template".
