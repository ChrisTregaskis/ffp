...
/**
 * Lambda handler for PUT /sessions/{id}/complete
 * Manually completes a session and triggers cascading completion.
 */
export const handler = withErrorHandling(
  async (event: APIGatewayProxyEventV2WithJWT): Promise<SessionStatusResponse> => {
    ...
    const result = await sessionService.completeSession(params.data.id, context);
