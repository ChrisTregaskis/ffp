...
**AWS Profile Auto-Loading**

To automatically load AWS credentials when working in this project, add this to your shell profile:

```bash
# For Bash (~/.bash_profile):
autoload_ffp_env() {
  local ffp_dir="/Users/christophertregaskis/Documents/FFP/ffp"
  if [[ $(pwd) == "$ffp_dir"* && -f "$ffp_dir/.env" ]]; then
    if [[ "$FFP_ENV_LOADED" != "true" ]]; then
      set -a; source "$ffp_dir/.env"; set +a
      export FFP_ENV_LOADED="true"
      echo "Loaded FFP environment (AWS_PROFILE=$AWS_PROFILE)"
    fi
  else
    if [[ "$FFP_ENV_LOADED" == "true" ]]; then
      unset AWS_PROFILE AWS_REGION FFP_ENV_LOADED
      echo "Unloaded FFP environment"
    fi
  fi
}
export PROMPT_COMMAND="autoload_ffp_env; $PROMPT_COMMAND"
autoload_ffp_env
...
chpwd_functions+=(autoload_ffp_env)
autoload_ffp_env
```

Reload your shell: `source ~/.bash_profile` or `source ~/.zshrc`

Check its referencing correct credentials: `aws sts get-caller-identity`
